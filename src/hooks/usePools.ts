  ...
  const parsePoolData = useCallback(
    (pools: PieceSwapPoolInfo[]): IPool[] => {
      ...
      if (obricSDK) {
        ...
        parsedPools = filteredPools.map((pool) => {
          ...
          // console.log('pools>>>', (pool.typeTag as StructTag).getAptosMoveTypeTag());
          return {
            id: (pool.typeTag as StructTag).getAptosMoveTypeTag(),
            address: (pool.typeTag as StructTag).address.toString(),
    ...
    async (poolAddress) => {
      ...
      if (poolStore && obricSDK) {
        ...
        if (coinInfo && poolInfo) {
          ...
          const { token0, token1, token0Reserve, token1Reserve, liquidity, address } = poolInfo;
